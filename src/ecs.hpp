...
template <typename... ComponentTypes> struct SceneView {
  ...
  struct Iterator {
    ...
    Iterator(Scene *scene, EntityID index, std::vector<uint8_t> &mask)
    ...
    bool valid_id() {
      for (size_t i = 0; i < _mask->size(); i++) {
        if ((_mask->at(i) &
             _scene->masks[current * (1 + componentCounter / 8)]) !=
            _mask->at(i))
          return false;
      }
  ...
  bool valid_index(EntityID index) {
    for (size_t i = 0; i < mask.size(); i++) {
      if ((mask.at(i) & _scene->masks[index * (1 + componentCounter / 8)]) !=
          mask.at(i))
        return false;
    }
